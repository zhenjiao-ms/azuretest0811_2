---
title: Create a web front end for your application using ASP.NET Core | Microsoft Azure
...
> [!NOTE]
> To complete this tutorial, you will need to [install .NET Core 1.0](https://www.microsoft.com/net/core#windows).
> 
> 

1. In Solution Explorer, right-click **Services** within the application project and choose **Add > New Service Fabric Service**.
   
    ![Adding a new service to an existing application](./media/service-fabric-add-a-web-frontend/vs-add-new-service.png)
2. On the **Create a Service** page, choose **ASP.NET Core** and give it a name.
   
    ![Choosing ASP.NET web service in the new service dialog](./media/service-fabric-add-a-web-frontend/vs-new-service-dialog.png)
3. The next page provides a set of ASP.NET Core project templates. Note that these are the same templates that you would see if you created an ASP.NET Core project outside of a Service Fabric application. For this tutorial, we will choose **Web API**. However, you can apply the same concepts to building a full web application.
    ...
    ![Choosing ASP.NET project type](./media/service-fabric-add-a-web-frontend/vs-new-aspnet-project-dialog.png)
...
3. Add `/api/values` to the location in the browser. This will invoke the `Get` method on the ValuesController in the Web API template. It will return the default response that is provided by the template--a JSON array that contains two strings:
    ...
    ![Default values returned from ASP.NET Core Web API template](./media/service-fabric-add-a-web-frontend/browser-aspnet-template-values.png)
...
2. Choose the **Visual C#** entry in the left navigation pane and then select the **Class Library** template. Ensure that the .NET Framework version is set to **4.5.2**.
    ...
    ![Creating an interface project for your stateful service](./media/service-fabric-add-a-web-frontend/vs-add-class-library-project.png)
...
4. Search for the **Microsoft.ServiceFabric.Services** package and install it.
    ...
    ![Adding the Services NuGet package](./media/service-fabric-add-a-web-frontend/vs-services-nuget-package.png)
...
1. In your stateful service, add a reference to the class library project that contains the interface.
    ...
    ![Adding a reference to the class library project in the stateful service](./media/service-fabric-add-a-web-frontend/vs-add-class-library-reference.png)
...
4. Press F5 again to run the modified application. As before, Visual Studio will automatically launch the browser to the root of the web project. Add the "api/values" path, and you should see the current counter value returned.
    ...
    ![The stateful counter value displayed in the browser](./media/service-fabric-add-a-web-frontend/browser-aspnet-counter-value.png)
   
    Refresh the browser periodically to see the counter value update.

> [!WARNING]
> The ASP.NET Core web server provided in the template, known as Kestrel, is [not currently supported for handling direct internet traffic](https://docs.asp.net/en/latest/fundamentals/servers.html#kestrel). For production scenarios, consider hosting your ASP.NET Core endpoints behind [API Management](https://azure.microsoft.com/en-us/services/api-management/) or another internet-facing gateway. Note that Service Fabric is not supported for deployment within IIS.
> 
> 
