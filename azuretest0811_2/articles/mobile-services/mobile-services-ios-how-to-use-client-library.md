---
title: How to Use iOS Client Library for Azure Mobile Services
...
This guide teaches you to perform common scenarios using the Azure Mobile Services [iOS SDK](https://developer.apple.com/xcode). If you are new to Mobile Services, first complete [Mobile Services Quick Start](mobile-services-ios-get-started.md) to configure your account, create a table, and create a mobile service.

> [!NOTE]
> This guide uses the latest [iOS Mobile Services SDK](https://go.microsoft.com/fwLink/?LinkID=266533&clcid=0x409). If your project uses an older version of the SDK, first upgrade the framework in Xcode.
> 
> 
...
This guide assumes that you have created a mobile service with a table. For more information see [Create a table](http://msdn.microsoft.com/library/windowsazure/jj193162.aspx), or reuse the `TodoItem` table created in [Mobile Services Quick Start](mobile-services-ios-get-started.md). This guide assumes that the table has the same schema as the tables in those tutorials. This guide also assumes that your Xcode references `WindowsAzureMobileServices.framework` and imports `WindowsAzureMobileServices/WindowsAzureMobileServices.h`.
...
Azure Mobile Services supports various identity providers. For a basic tutorial, see [Authentication](/develop/mobile/tutorials/get-started-with-users-ios).
...
* **Client-managed Login**: The *app* requests a token from the identity provider and presents this token to Azure Mobile Services for authentication.

When authentication succeeds, you get back a user object with a user ID value and the auth token. To use this user ID to authorize users, see [Service-side Authorization](mobile-services-javascript-backend-service-side-authorization.md). To restrict table access to only authenticated users, see [Permissions](http://msdn.microsoft.com/library/windowsazure/jj193161.aspx).

### Server-managed Login
Here is how you can add server-managed login to the [Mobile Services Quick Start](mobile-services-ios-get-started.md) project; you may use similar code for your other projects. For more information and to see an end-to-end example in action, see [Authentication](/develop/mobile/tutorials/get-started-with-users-ios).
...
The following example uses the [Live Connect SDK](http://go.microsoft.com/fwlink/p/?LinkId=301960) to enable single sign-on for iOS apps. It assumes that you have a **LiveConnectClient** instance named `liveClient` in the controller and the user is logged in.
...
Let's see how you may cache tokens in the [Mobile Services Quick Start](mobile-services-ios-get-started.md) project; you may apply similar steps to any project. [!INCLUDE [mobile-services-ios-authenticate-app-with-token](../../includes/mobile-services-ios-authenticate-app-with-token.md)]
