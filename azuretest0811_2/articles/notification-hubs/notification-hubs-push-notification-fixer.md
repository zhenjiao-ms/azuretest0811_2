...
3. **Google Cloud Messaging (GCM) configuration** 
    ...
    ![](./media/notification-hubs-diagnosing/GCMConfigure.png)
...
If you are using templates then ensure that you are following the guidelines described at [Template guidance](https://msdn.microsoft.com/library/dn530748.aspx). 
...
> [!NOTE]
> Since we do the processing in parallel, we don’t guarantee the order in which the notifications will be delivered. 
> 
> 
...
Source - [APNS guidance](https://developer.apple.com/library/ios/documentation/NetworkingInternet/Conceptual/RemoteNotificationsPG/Chapters/ApplePushService.html#//apple_ref/doc/uid/TP40008194-CH100-SW4) & [GCM guidance](http://developer.android.com/google/gcm/adv.html)
...
1. **PNS developer portal**
    ...
    Verify them at the respective PNS developer portal (APNS, GCM, WNS etc) using our [Getting Started Tutorials](notification-hubs-windows-store-dotnet-get-started.md).
2. **Azure Classic portal**
    ...
    ![](./media/notification-hubs-diagnosing/PortalConfigure.png)
...
1. **Visual Studio**
    ...
    ![](./media/notification-hubs-diagnosing/VSServerExplorer.png)
    ...
    ![](./media/notification-hubs-diagnosing/VSRegistrations.png)
   
   > [!NOTE]
   > Visual Studio functionality to edit registrations should only be used during dev/test with limited number of registrations. If there arises a need to fix your registrations in bulk, consider using the Export/Import registration functionality described here - [Export/Import Registrations] (https://msdn.microsoft.com/library/dn790624.aspx)
   > 
2. **Service Bus explorer**
   
    Many customers use ServiceBus explorer described here - [ServiceBus Explorer](http://msdn.microsoft.com/library/dn530751.aspx) for viewing and managing their notification hub. It is an open source project available from code.microsoft.com - [ServiceBus Explorer code](https://code.msdn.microsoft.com/windowsazure/Service-Bus-Explorer-f2abca5a)
...
1. **Azure Classic Portal**
    ...
    ![](./media/notification-hubs-diagnosing/PortalTestNotification.png)
2. **Visual Studio**
   
    You can also send test notifications from the comforts of Visual Studio:
   
    ![](./media/notification-hubs-diagnosing/VSTestNotification.png)
...
To get an insight into the PNS errors, we have introduced a property called [EnableTestSend feature](http://msdn.microsoft.com/library/microsoft.servicebus.notifications.notificationhubclient.enabletestsend.aspx). This property is automatically enabled when you send test messages from the portal or Visual Studio client and therefore allows you to see detailed debugging information. You can use this via APIs taking the example of the .NET SDK where it is available now and will be added to all client SDKs eventually. To use this with the REST call, simply append a querystring parameter called "test" at the end of your send call e.g. 
...
1. **Use Azure Classic Portal**
    ...
    ![](./media/notification-hubs-diagnosing/PortalDashboard.png)
    ...
    ![](./media/notification-hubs-diagnosing/PortalMonitoring.png)
