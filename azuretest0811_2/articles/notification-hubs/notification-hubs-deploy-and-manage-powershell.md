...
* A computer with Azure PowerShell. For instructions, see [Install and configure Azure PowerShell](../powershell-install-configure.md).
...
You can use the [Get-AzureSBAuthorizationRule](https://msdn.microsoft.com/library/azure/dn495113.aspx) cmdlet included with Azure PowerShell to retrieve an authorization rule that's used to provide a connection string. We'll store a reference to the `NamespaceManager` instance in the `$NamespaceManager` variable. We will use `$NamespaceManager` to provision a notification hub.
...
To provision a new notification hub, use the [.NET API for Notification Hubs](https://msdn.microsoft.com/library/azure/mt414893.aspx).
