...
SQL Data Warehouse offers several indexing options including [clustered columnstore indexes](https://msdn.microsoft.com/library/gg492088.aspx), [clustered indexes and nonclustered indexes](https://msdn.microsoft.com/library/ms190457.aspx).  In addition, it also offers a no index option also known as [heap](https://msdn.microsoft.com/library/hh213609.aspx).  This article covers the benefits of each index type as well as tips to getting the most performance out of your indexes. See [create table syntax](https://msdn.microsoft.com/library/mt203953.aspx) for more detail on how to create a table in SQL Data Warehouse.
...
Below is an example of how to allocate more memory to a user by increasing their resource class.  For more information about resource classes and how to create a new user can be found in the [concurrency and workload management](sql-data-warehouse-develop-concurrency.md) article.
...
Alternatively, instead of rebuilding the index, you could copy the table to a new table using [CTAS](sql-data-warehouse-develop-ctas.md).  Which way is best? For large volumes of data, [CTAS](sql-data-warehouse-develop-ctas.md) is usually faster than [ALTER INDEX](https://msdn.microsoft.com/library/ms188388.aspx). For smaller volumes of data, [ALTER INDEX](https://msdn.microsoft.com/library/ms188388.aspx) is easier to use and won't require you to swap out the table.  See **Rebuilding indexes with CTAS and partition switching** below for more details on how to rebuild indexes with CTAS.
...
Rebuilding an index in SQL Data Warehouse is an offline operation.  For more information about rebuilding indexes, see the ALTER INDEX REBUILD section in [Columnstore Indexes Defragmentation](https://msdn.microsoft.com/library/dn935013.aspx#Anchor_1) and the syntax topic [ALTER INDEX](https://msdn.microsoft.com/library/ms188388.aspx).
...
This example uses [CTAS](sql-data-warehouse-develop-ctas.md) and partition switching to rebuild a table partition. 
...
For more details about re-creating partitions using `CTAS`, see the [Partition](sql-data-warehouse-tables-partition.md) article.

## Next steps
To learn more, see the articles on [Table Overview](sql-data-warehouse-tables-overview.md), [Table Data Types](sql-data-warehouse-tables-data-types.md), [Distributing a Table](sql-data-warehouse-tables-distribute.md),  [Partitioning a Table](sql-data-warehouse-tables-partition.md), [Maintaining Table Statistics](sql-data-warehouse-tables-statistics.md) and [Temporary Tables](sql-data-warehouse-tables-temporary.md).  To learn more about best practices, see [SQL Data Warehouse Best Practices](sql-data-warehouse-best-practices.md).
