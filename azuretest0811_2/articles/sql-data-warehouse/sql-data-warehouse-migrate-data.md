---
title: Migrate your data to SQL Data Warehouse | Microsoft Azure
...
[ADF Copy](../data-factory/data-factory-copy-activity.md) is part of [Azure Data Factory](http://azure.microsoft.com/services/data-factory/). You can use ADF Copy to export your data to flat files residing on local storage, to remote flat files held in Azure blob storage or directly into SQL Data Warehouse.

If your data starts in flat files then you will first need to transfer it to Azure storage blob before initiating a load it into SQL Data Warehouse. Once the data is transferred into Azure blob storage you can choose to use [ADF Copy](../data-factory/data-factory-copy-activity.md) again to push the data into SQL Data Warehouse.
...
Head over to the following article for some great [ADF Copy examples](../data-factory/data-factory-copy-activity-examples.md).
...
SSIS connects to SQL Data Warehouse just as it would connect to a SQL Server deployment. However, your connections will need to be using an ADO.NET connection manager. You should also take care to configure the "Use bulk insert when available" setting to maximize throughput. Please refer to the [ADO.NET destination adapter](https://msdn.microsoft.com/library/bb934041.aspx) article to learn more about this property
...
For more information consult the [SSIS documentation](https://msdn.microsoft.com/library/ms141026.aspx).
...
For more information, see [Use bcp to load data into SQL Data Warehouse](sql-data-warehouse-load-with-bcp.md).
...
You may want to consider using [ExpressRoute](http://azure.microsoft.com/services/expressroute/) to speed up the transfer. [ExpressRoute](http://azure.microsoft.com/services/expressroute/) provides you with an established private connection to Azure so the connection does not go over the public internet. This is by no means a mandatory step. However, it will improve throughput when pushing data to Azure from an on-premises or co-location facility.

The benefits of using [ExpressRoute](http://azure.microsoft.com/services/expressroute/) are:
...
[ExpressRoute](http://azure.microsoft.com/services/expressroute/) is beneficial for a number of scenarios; not just the migration.

Interested? For more information and pricing please visit the [ExpressRoute documentation](http://azure.microsoft.com/documentation/services/expressroute/).
...
### [AZCopy](../storage/storage-use-azcopy.md) utility
The [AZCopy](../storage/storage-use-azcopy.md) utility is a great tool for getting your data transferred into Azure Storage Blobs. It is designed for small (MB++) to very large (GB++) data transfers. [AZCopy](../storage/storage-use-azcopy.md) has also been designed to provide good resilient throughput when transferring data to Azure and so is a great choice for the data transfer step. Once transferred you can load the data using PolyBase into SQL Data Warehouse. You can also incorporate AZCopy into your SSIS packages using an "Execute Process" task.

To use AZCopy you will first need to download and install it. There is a [production version](http://aka.ms/downloadazcopy/) and a [preview version](http://aka.ms/downloadazcopypr/) available.
...
Full documentation available: [AZCopy](../storage/storage-use-azcopy.md).
...
To learn more about loading data with PolyBase, see [Use PolyBase to load data into SQL Data Warehouse](sql-data-warehouse-get-started-load-with-polybase.md).

## Next steps
For more about migration, see [Migrate your solution to SQL Data Warehouse](sql-data-warehouse-overview-migrate.md).
For more development tips, see [development overview](sql-data-warehouse-develop-overview.md).
