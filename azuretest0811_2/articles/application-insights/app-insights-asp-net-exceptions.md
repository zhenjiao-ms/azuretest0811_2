---
title: Diagnose failures and exceptions in ASP.NET apps with Application Insights
...
By monitoring your application with [Visual Studio Application Insights](app-insights-overview.md), you can correlate failed requests with exceptions and other events at both the client and server, so that you can quickly diagnose the causes.

To monitor an ASP.NET app, you have to [add Application Insights SDK](app-insights-asp-net.md) to your application, or [install Status Monitor on your IIS server](app-insights-monitor-performance-live-website-now.md), or, if your app is an Azure Web App, add the [Application Insights Extension](../azure-portal/insights-perf-analytics.md).
...
## Dependency failures
A *dependency* is a service that your application calls, typically through a REST API or database connection. [Application Insights Status Monitor](app-insights-monitor-performance-live-website-now.md) automatically monitors a variety of types of dependency call, measuring call duration and success or failure. 

To get dependency data, you have to [install Status Monitor](app-insights-monitor-performance-live-website-now.md) on your IIS server, or if your app is an Azure Web App, use the [Application Insights Extension](../azure-portal/insights-perf-analytics.md). 
...
* Insert code in your application to call [Microsoft.ApplicationInsights.TrackTrace()](app-insights-api-custom-events-metrics.md). Send the POST data in the message parameter. There is a limit to the permitted size, so you should try to send just the essential data.
...
If you have [installed Status Monitor](app-insights-monitor-performance-live-website-now.md) on your server, you can get a chart of the exceptions rate, measured by .NET. This includes both handled and unhandled .NET exceptions.
