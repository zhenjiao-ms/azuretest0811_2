...
If you haven't yet [set up Application Insights for your project](app-insights-overview.md), do that now.
...
Custom events show up both in [Diagnostic Search](app-insights-diagnostic-search.md) and in [Metric Explorer](app-insights-metrics-explorer.md). You can send them from devices, web pages and server applications. They can be used both for diagnostic purposes and to [understand usage patterns](app-insights-api-custom-events-metrics.md).
...
Page view telemetry is sent by the trackPageView() call in [the JavaScript snippet you insert in your web pages](app-insights-web-track-usage.md). Its main purpose is to contribute to the counts of page views that you see on the overview page.
...
In some cases, you need to [insert a few lines of code](app-insights-asp-net-exceptions.md) to make sure your exceptions are being caught automatically.
...
Application Insights reports unhandled exceptions where it can, from devices, [web browsers](app-insights-web-track-usage.md), or web servers, whether instrumented by [Status Monitor](app-insights-monitor-performance-live-website-now.md) or [Application Insights SDK](app-insights-asp-net.md). 

However, it isn't always able to do this in some cases because the .NET framework catches the exceptions.  To make sure you see all exceptions, you therefore have to write a small exception handler. The best procedure varies with the technology. See [Exception telemetry for ASP.NET](app-insights-asp-net-exceptions.md) for details. 
...
Request telemetry is sent automatically when you [install Status Monitor on your web server](app-insights-monitor-performance-live-website-now.md), or when you [add Application Insights to your web project](app-insights-asp-net.md). It also feeds into the request and response time charts in Metric Explorer and on the Overview page.
