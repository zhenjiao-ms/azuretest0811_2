---
title: Build an HBase application using Maven, and deploy to Windows-based HDInsight | Microsoft Azure
...
5. Create a new directory named **conf** in the **hbaseapp** directory. In the **conf** directory, create a new file named **hbase-site.xml** and use the following as the contents:
   ...
   > [!NOTE]
   > This is a very minimal hbase-site.xml file, and it contains the bare minimum settings for the HDInsight cluster.
   > 
...
6. To search for entries in the table, use the following command:
    ...
    Using **fabrikam.com** for the `-emailRegex` value will return the users that have **fabrikam.com** in the email field. Since this search is implemented by using a regular expression-based filter, you can also enter regular expressions, such as **^r**, which will return entries where the email begins with the letter 'r'.
