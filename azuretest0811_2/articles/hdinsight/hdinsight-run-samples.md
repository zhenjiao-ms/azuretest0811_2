---
title: Run the Hadoop samples in HDInsight | Microsoft Azure
...
* [Introduction to Azure HDInsight](hdinsight-hadoop-introduction.md)
...
1. Open **Windows PowerShell ISE**. For instructions, see [Install and configure Azure PowerShell](../powershell-install-configure.md).
...
In the example, the mapper and the reducer are executables that read the input from [stdin](https://msdn.microsoft.com/library/3x292kth.aspx) (line-by-line) and emit the output to [stdout](https://msdn.microsoft.com/library/3x292kth.aspx). The program counts all of the words in the text.

When an executable is specified for **mappers**, each mapper task launches the executable as a separate process when the mapper is initialized. As the mapper task runs, it converts its input into lines, and feeds the lines to the [stdin](https://msdn.microsoft.com/library/3x292kth.aspx) of the process.
...
When an executable is specified for **reducers**, each reducer task launches the executable as a separate process when the reducer is initialized. As the reducer task runs, it converts its input key/values pairs into lines, and it feeds the lines to the [stdin](https://msdn.microsoft.com/library/3x292kth.aspx) of the process.

In the meantime, the reducer collects the line-oriented output from the [stdout](https://msdn.microsoft.com/library/3x292kth.aspx) of the process. It converts each line to a key/value pair, which is collected as the output of the reducer. By default, the prefix of a line up to the first Tab character is the key, and the remainder of the line (excluding the Tab character) is the value.
...
* [Get started using Hadoop with Hive in HDInsight to analyze mobile handset use](hdinsight-hadoop-linux-tutorial-get-started.md)
* [Use Pig with Hadoop on HDInsight](hdinsight-use-pig.md)
* [Use Hive with Hadoop on HDInsight](hdinsight-use-hive.md)
* [Submit Hadoop Jobs in HDInsight](hdinsight-submit-hadoop-jobs-programmatically.md)
* [Azure HDInsight SDK documentation](https://msdn.microsoft.com/library/azure/dn479185.aspx)
* [Debug Hadoop in HDInsight: Error messages](hdinsight-debug-jobs.md)
...
The mapper code in the cat.cs file uses a [StreamReader](http://msdn.microsoft.com/library/system.io.streamreader.aspx) object to read the characters of the incoming stream to the console, which then writes the stream to the standard output stream with the static [Console.Writeline](http://msdn.microsoft.com/library/system.console.writeline) method.
...
The reducer code in the wc.cs file uses a [StreamReader](http://msdn.microsoft.com/library/system.io.streamreader.aspx)   object to read characters from the standard input stream that have been output by the cat.exe mapper. As it reads the characters with the [Console.Writeline](http://msdn.microsoft.com/library/system.console.writeline) method, it counts the words by counting spaces and end-of-line characters at the end of each word. It then writes the total to the standard output stream with the [Console.Writeline](http://msdn.microsoft.com/library/system.console.writeline) method.
