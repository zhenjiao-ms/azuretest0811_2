---
title: Script Action development with HDInsight | Microsoft Azure
...
Script Action can be deployed from the Azure Portal, Azure PowerShell or by using the HDInsight .NET SDK.  For more information, see [Customize HDInsight clusters using Script Action](hdinsight-hadoop-customize-cluster.md).
...
This statement sets the environment variable **MDS_RUNNER_CUSTOM_CLUSTER** to the value 'true' and also sets the scope of this variable to be machine-wide. At times it is important that environment variables are set at the appropriate scope – machine or user. Refer [here](https://msdn.microsoft.com/library/96xafkes\(v=vs.110\).aspx) for more information on setting environment variables.
