---
title: PHP web and worker roles | Microsoft Azure
...
3. (OPTIONAL) If your PHP runtime uses the [Microsoft Drivers for PHP for SQL Server](http://php.net/sqlsrv), you will need to configure your web role to install [SQL Server Native Client 2012](http://msdn.microsoft.com/sqlserver/aa937733.aspx) when it is provisioned. To do this, add the [sqlncli.msi x64 installer](http://go.microsoft.com/fwlink/?LinkID=239648) to the `bin` folder in your web role's root directory. The startup script described in the next step will silently run the installer when the role is provisioned. If your PHP runtime does not use the Microsoft Drivers for PHP for SQL Server, you can remove the following line from the script shown in the next step:
   
        msiexec /i sqlncli.msi /qn IACCEPTSQLNCLILICENSETERMS=YES
4. Define a startup task that configures [Internet Information Services (IIS)](http://www.iis.net/) to use your PHP runtime to handle requests for `.php` pages. To do this, open the `setup_web.cmd` file (in the `bin` file of your web role's root directory) in a text editor and replace its contents with the following script:
...
3. (OPTIONAL) If your PHP runtime uses [Microsoft Drivers for PHP for SQL Server](http://php.net/sqlsrv), you will need to configure your worker role to install [SQL Server Native Client 2012](http://msdn.microsoft.com/sqlserver/aa937733.aspx) when it is provisioned. To do this, add the [sqlncli.msi x64 installer](http://go.microsoft.com/fwlink/?LinkID=239648) to the worker role's root directory. The startup script described in the next step will silently run the installer when the role is provisioned. If your PHP runtime does not use the Microsoft Drivers for PHP for SQL Server, you can remove the following line from the script shown in the next step:
