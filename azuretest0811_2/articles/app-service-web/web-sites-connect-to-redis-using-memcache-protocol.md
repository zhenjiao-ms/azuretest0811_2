---
title: Connect a web app in Azure App Service to Redis Cache via the Memcache protocol | Microsoft Azure
...
* [Provision an instance of the Azure Redis Cache Service](../redis-cache/cache-dotnet-how-to-use-azure-redis-cache.md#create-a-cache)
* [Deploy a Scalable WordPress site in Azure](http://bit.ly/1t0KxBQ)
...
In order to configure Memcache shim, you must create three app settings. This can be done using a variety of methods including the [Azure Portal](http://go.microsoft.com/fwlink/?LinkId=529715), the [classic portal](http://portal.azure.com), the [Azure PowerShell Cmdlets](/downloads) or the [Azure Command-Line Interface](/downloads). For the purposes of this post, I’m going to use the [Azure Portal](../powershell-install-configure.md) to set the app settings. The following values can be retrieved from **Settings** blade of your Redis Cache instance.
...
Browse to [PECL](http://pecl.php.net). Under the caching category, click [memcache](http://pecl.php.net/package/memcache). Under the downloads column click the DLL link.
...
> [!NOTE]
> Follow the instructions in this blog on [How to enable a Site Extension in Web Apps](http://blog.syntaxc4.net/post/2015/02/05/how-to-enable-a-site-extension-in-azure-websites.aspx) to install Visual Studio Team Services.
> 
> 
...
> [!NOTE]
> This step assumes that redis is installed locally on your development machine. [Install Redis locally using these instructions](http://redis.io/download#installation).
> 
> 
...
## Conclusion
Congratulations! The WordPress app now has a centralized in-memory cache to aid in increasing throughput. Remember, the Web Apps Memcache Shim can be used with any Memcache client regardless of programming language or application framework. To provide feedback or to ask questions about the Web Apps Memcache shim, post to [MSDN Forums](https://social.msdn.microsoft.com/Forums/home?forum=windowsazurewebsitespreview) or [Stackoverflow](http://stackoverflow.com/questions/tagged/azure-web-sites).
