...
| **Deploy your site** |[Deploy a web app in Azure App Service](web-sites-deploy.md) |
...
4. Create a new web app in the Azure Portal and publish the WordPress backup.
   ...
   3. Use the [Deploy a web app in Azure App Service](web-sites-deploy.md) information to enable the deployment method you wish to use, and then deploy your WordPress backup to your web app in Azure App Service.
...
| To do this... | Use this... |
| --- | --- |
| **Set App Service plan mode, size, and enable scaling** |[Scale a web app in Azure App Service](web-sites-scale.md) |
...
| **Improve performance** |<ul><li><p>[Disable the ARR cookie](http://ppe.blogs.msdn.com/b/windowsazure/archive/2013/11/18/disabling-arr-s-instance-affinity-in-windows-azure-web-sites.aspx) - can improve performance when running WordPress on multiple Web Apps instances</p></li><li><p>Enable caching. [Redis cache](http://msdn.microsoft.com/library/azure/dn690470.aspx) can be used with the [Redis object cache WordPress plugin](https://wordpress.org/plugins/redis-object-cache/), or use one of the other caching offerings from the [Azure Store](/gallery/store/)</p></li><li><p>[How to make WordPress faster with Wincache](http://ruslany.net/2010/03/make-wordpress-faster-on-iis-with-wincache-1-1/) - Wincache is enabled by default for Web Apps</p></li><li><p>[Scale a web app in Azure App Service](web-sites-scale.md) and use [ClearDB High Availability Routing](http://www.cleardb.com/developers/cdbr/introduction) or [MySQL Cluster CGE](http://www.mysql.com/products/cluster/)</p></li></ul> |
| **Use blobs for storage** |<ol><li><p>[Create an Azure Storage account](../storage/storage-create-storage-account.md)</p></li><li><p>Learn how to [Use the Content Distribution Network (CDN)](../cdn/cdn-overview.md) to geo-distribute data stored in blobs.</p></li><li><p>Install and configure the [Azure Storage for WordPress plugin](https://wordpress.org/plugins/windows-azure-storage/).</p><p>For detailed setup and configuration information for the plugin, see the [user guide](http://plugins.svn.wordpress.org/windows-azure-storage/trunk/UserGuide.docx).</p> </li></ol> |
...
| **Configure a custom domain name** |[Configure a custom domain name in Azure App Service](web-sites-custom-domain-name.md) |
| **Enable HTTPS for a custom domain name** |[Enable HTTPS for a web app in Azure App Service](web-sites-configure-ssl-certificate.md) |
| **Load balance or geo-distribute your site** |[Route traffic with Azure Traffic Manager](../traffic-manager/traffic-manager-overview.md). If you are using a custom domain, see [Configure a custom domain name in Azure App Service](web-sites-custom-domain-name.md) for information on using Traffic Manager with custom domain names |
| **Enable automated backups** |[Back up a web app in Azure App Service](web-sites-backup.md) |
| **Enable diagnostic logging** |[Enable diagnostics logging for web apps in Azure App Service](web-sites-enable-diagnostic-log.md) |
