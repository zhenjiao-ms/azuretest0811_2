...
1. Add the following two methods to the `Program` class. These will be used to execute REST APIs and display responses to the console.
...
1. Add the following code to your `Main` method to generate an authentication token with the authentication parameters you collected:
...
1. The campaign must be activated before it can be pushed to any devices. We saved the ID for the new campaign in the `NewCampaignID` variable. We will use this as a URI path parameter to activate the campaign using the [Activate campaign](https://msdn.microsoft.com/library/azure/mt683745.aspx) REST API. This should change the state of the campaign to **scheduled** even though it will only be pushed manually with the APIs.
...
2. To push the campaign we need to provide the device Ids for the users that we want to receive the notification. We will use the [Query devices](https://msdn.microsoft.com/library/azure/mt683826.aspx) REST API to get all device Ids. We will add each device Id to the list if it has associated **user_name** appInfo.
...
3. Lastly, we will push the campaign to all device Ids in our list using the [Push campaign](https://msdn.microsoft.com/library/azure/mt683734.aspx) REST API. This is an **in-app** notification. So the app will have to be running on the device in order for it to be received by the user.
...
4. Build and run your console app. Your output should be similar to the following:
