---
title: Use Azure Media Services to Stream your HLS content Protected with Apple FairPlay
...
4. Configure the content key’s authorization policy. When creating the content key authorization policy, you need to specify the following: 
   ...
   * FairPlay policy options configuration. For details on how to configure FairPlay, see ConfigureFairPlayPolicyOptions() method in the sample below.
     ...
     > [!NOTE]
     > In most cases, you would want to configure FairPlay policy options only once, since you will only have one set of certification and ASK.
...
> [!NOTE]
> Azure Media Player doesn’t support FairPlay playback out of the box. Customers need to obtain the sample player from Apple developer account in order to get FairPlay playback on MAC OSX. 
> 
> 
