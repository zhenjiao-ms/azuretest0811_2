---
title: SAP NetWeaver on Linux virtual machines (VMs) – Deployment Guide | Microsoft Azure
...
This white paper describes step by step how a Azure Virtual Machine is prepared for the deployment of SAP NetWeaver based applications. It assumes that the information contained in the [Planning and Implementation Guide](virtual-machines-linux-sap-planning-guide.md "SAP NetWeaver on Linux virtual machines \(VMs\) – Planning and Implementation Guide") is known. If not, the respective document should be read first.
...
Azure Storage Accounts or Azure Virtual Networks as such should have been created and configured already. How to create and configure them is covered in the [Planning and Implementation Guide](virtual-machines-linux-sap-planning-guide.md "SAP NetWeaver on Linux virtual machines \(VMs\) – Planning and Implementation Guide").
...
#### Resource Groups
Resource groups are a new concept that contain all resources that have the same lifecycle e.g. they are created and deleted at the same time. Read [this article](../resource-group-overview.md) for more information about resource groups. 
...
* SAP Note [2015553](https://service.sap.com/sap/support/notes/2015553) listing prerequisites to be supported by SAP when deploying SAP software into Microsoft Azure.
* SAP Note [1999351](https://service.sap.com/sap/support/notes/1999351) containing additional troubleshooting information for the Enhanced Azure Monitoring for SAP.
* SAP Note [2178632](https://service.sap.com/sap/support/notes/2178632) containing detail information on all available monitoring metrics for SAP on Microsoft Azure. 
* SAP Note [1409604](https://service.sap.com/sap/support/notes/1409604) containing the required SAP Host Agent version for Windows on Microsoft Azure when deploying on the new Azure Resource Manager.
* SAP Note [2191498](https://service.sap.com/sap/support/notes/2191498) containing the required SAP Host Agent version for Linux on Microsoft Azure when deploying on the new Azure Resource Manager.
* SAP Note [2243692](https://service.sap.com/sap/support/notes/2243692) containing information about licensing for SAP on Linux on Azure
* SAP Note [1984787](https://service.sap.com/sap/support/notes/1984787) containing general information about SUSE LINUX Enterprise Server 12
* SAP Note [2002167](https://service.sap.com/sap/support/notes/2002167) containing general information Red Hat Enterprise Linux 7.x
* [SCN](https://wiki.scn.sap.com/wiki/display/HOME/SAPonLinuxNotes) that contains all required SAP Notes for Linux
* SAP specific PowerShell cmdlets that are part of the [Azure PowerShell](../powershell-install-configure.md)
* SAP specific Azure CLI that are part of the [Azure CLI](../xplat-cli-install.md)
...
* [SAP NetWeaver on Azure virtual machines (VMs) – Deployment Guide (this document)](virtual-machines-linux-sap-deployment-guide.md "SAP NetWeaver on Linux virtual machines \(VMs\) – Deployment Guide")
...
For more details see chapter [Scenario 1: Deploying a VM out of the Azure Marketplace for SAP](virtual-machines-linux-sap-deployment-guide.md#db477013-9060-4602-9ad4-b0316f8bb281 "Scenario 1: Deploying a VM out of the Azure Marketplace for SAP").
...
> 
> To prepare a Windows image that can be used to deploy multiple virtual machines, the Windows settings (like Windows SID and hostname) must be abstracted/generalized on the on-premises VM. This can be done using sysprep as described on [https://technet.microsoft.com/library/cc721940.aspx](https://technet.microsoft.com/library/cc721940.aspx).
> 
> ![Linux](./media/virtual-machines-linux-sap-shared/Linux.png) Linux
> 
> To prepare a Linux image that can be used to deploy multiple virtual machines, some Linux settings must be abstracted/generalized on the on-premises VM. This can be done using waagent -deprovision as described in [this article](virtual-machines-linux-capture-image.md) or in [this article](virtual-machines-linux-agent-user-guide.md#command-line-options).
> 
> 

- - -
You can set up your database content by either using the SAP Software Provision Manager to install a new SAP System, restore a database backup from a VHD that is attached to the virtual machine or directly restore a database backup from Azure Storage if the DBMS supports it. (see the [DBMS Deployment Guide](virtual-machines-linux-sap-dbms-guide.md "SAP NetWeaver on Linux virtual machines \(VMs\) – DBMS Deployment Guide")). If you have already installed an SAP system in your on-premises VM (especially for 2-Tier systems), you can adapt the SAP system settings after the deployment of the Azure VM through the System Rename procedure supported by the SAP Software Provisioning Manager (SAP Note [1619720](https://service.sap.com/sap/support/notes/1619720)). Otherwise you can install the SAP software later after the deployment of the Azure VM.

For more details see chapter [Scenario 2: Deploying a VM with a custom image for SAP](virtual-machines-linux-sap-deployment-guide.md#54a1fc6d-24fd-4feb-9c57-ac588a55dff2 "Scenario 2: Deploying a VM with a custom image for SAP").

#### Moving a VM from on-premises to Microsoft Azure with a non-generalized disk
You plan to move a specific SAP system from on-premises to Microsoft Azure. This can be done by uploading the VHD which contains the OS, the SAP binaries and eventual DBMS binaries plus the VHDs with the data and log files of the DBMS to Microsoft Azure. In opposite to the scenario described in chapter [Deploying a VM with a custom image](virtual-machines-linux-sap-deployment-guide.md#3688666f-281f-425b-a312-a77e7db2dfab "Deploying a VM with a custom image") above, you keep the hostname, SAP SID and SAP user accounts in the Azure VM as they were configured in the on-premises environment. Therefore, generalizing the operating system is not necessary. This case will mostly apply for cross-premises scenarios where a part of the SAP landscape is run on-premises and parts on Microsoft Azure.

For more details see chapter [Scenario 3: Moving a VM from on-premises using a non-generalized Azure VHD with SAP](virtual-machines-linux-sap-deployment-guide.md#a9a60133-a763-4de8-8986-ac0fa33aa8c1 "Scenario 3: Moving a VM from on-premises using a non-generalized Azure VHD with SAP").

### <a name="db477013-9060-4602-9ad4-b0316f8bb281"></a>Scenario 1: Deploying a VM out of the Azure Marketplace for SAP
Microsoft Azure offers the possibility to deploy a VM instance out of the Azure Marketplace, which offers some standard OS images of Windows Server and different Linux distributions. It is also possible to deploy an image that includes DBMS SKUs e.g. SQL Server. For details using those images with DBMS SKUs please refer to the [DBMS Deployment Guide](virtual-machines-linux-sap-dbms-guide.md "SAP NetWeaver on Linux virtual machines \(VMs\) – DBMS Deployment Guide")

The SAP specific sequence of steps deploying a VM out of the Azure Marketplace would look like:

![Flowchart of VM deployment for SAP systems using a VM image from Azure Marketplace](./media/virtual-machines-linux-sap-deployment-guide/100-deploy-vm-image.png)
...
1. Basics
   ...
   2. Username and password/SSH public key: Enter the username and password of the user that is created during the provisioning. For a Linux virtual machine, you can also enter the public SSH key that you want to use to login to the machine using SSH.
   3. Subscription: Select the subscription that you want to use to provision the new virtual machine.
   4. Resource Group: The name of the resource group. You can either insert the name of a new resource group or of a resource group that already exists
   5. Location: Select the location where the new virtual machine should be deployed. If you want to connect the virtual machine to your on-premises network, make sure to select the location of the Virtual Network that connects Azure to your on-premises network. For more details, see chapter [Microsoft Azure Networking](virtual-machines-linux-sap-planning-guide.md#61678387-8868-435d-9f8c-450b2424f5bd "Microsoft Azure Networking") in the [Planning Guide](virtual-machines-linux-sap-planning-guide.md "SAP NetWeaver on Linux virtual machines \(VMs\) – Planning and Implementation Guide").
2. Size: Please read SAP Note [1928533](https://service.sap.com/sap/support/notes/1928533) for a list of supported VM types. Please also make sure to select the correct type if you want to use Premium Storage. Not all VM types support Premium Storage. See chapter [Storage: Microsoft Azure Storage and Data Disks](virtual-machines-linux-sap-planning-guide.md#a72afa26-4bf4-4a25-8cf7-855d6032157f "Storage: Microsoft Azure Storage and Data Disks") and [Azure Premium Storage](virtual-machines-linux-sap-planning-guide.md#ff5ad0f9-f7f4-4022-9102-af07aef3bc92 "Azure Premium Storage") in the [Planning Guide](virtual-machines-linux-sap-planning-guide.md "SAP NetWeaver on Linux virtual machines \(VMs\) – Planning and Implementation Guide") for more details.
3. Settings
   1. Storage Account: You can select an existing storage account or create a new one. Please read chapter [Microsoft Azure Storage](virtual-machines-linux-sap-dbms-guide.md#10b041ef-c177-498a-93ed-44b3441ab152 "Microsoft Azure Storage") of the [DBMS Guide](virtual-machines-linux-sap-dbms-guide.md "SAP NetWeaver on Linux virtual machines \(VMs\) – DBMS Deployment Guide") for more details on the different storage types. Note that not all storage types are supported for running SAP applications.
   2. Virtual network and Subnet: Select the virtual network that is connected to your on-premises network if you want to integrate the virtual machine into your intranet.
   3. Public IP address: Select the Public IP address that you want to use or enter the parameters to create a new Public IP address. You can use a Public IP address to access your virtual machine over the internet. Make sure to also create a Network Security Group to filter access to your virtual machine.
   4. Network Security Group: see [What is a Network Security Group (NSG)](../virtual-network/virtual-networks-nsg.md) for more details.
   5. Monitoring: You can disable the diagnostics setting. It will be enabled automatically when you run the commands to enable the Azure Enhanced Monitoring as described in chapter [Configure Monitoring](virtual-machines-linux-sap-deployment-guide.md#ec323ac3-1de9-4c3a-b770-4ff701def65b "Configure Monitoring").
   6. Availability: Select an Availablility Set or enter the parameters to create a new Availablility Set. For more information see chapter [Azure Availability Sets](virtual-machines-linux-sap-planning-guide.md#18810088-f9be-4c97-958a-27996255c665 "Azure Availability Sets").
...
You can also create a deployment using one of the SAP templates published in the [azure-quickstart-templates github repository](https://github.com/Azure/azure-quickstart-templates). Or you can create a virtual machine using the [Azure Portal](virtual-machines-windows-hero-tutorial.md), [PowerShell](virtual-machines-windows-ps-create.md) or [Azure CLI](virtual-machines-linux-quick-create-cli.md) manually.

* [2-tier configuration (only one virtual machine) template](https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2FAzure%2Fazure-quickstart-templates%2Fmaster%2Fsap-2-tier-marketplace-image%2Fazuredeploy.json)
  Use this template if you want to create a 2-tier system using only one virtual machine.
* [3-tier configuration (multiple virtual machines) template](https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2FAzure%2Fazure-quickstart-templates%2Fmaster%2Fsap-3-tier-marketplace-image%2Fazuredeploy.json)
...
* storageType: (2-tier template only) Type of storage that should be used 
  * For bigger systems, the usage of Premium Storage is highly recommended. For more information about the different storage types, read 
    * [Microsoft Azure Storage](virtual-machines-linux-sap-dbms-guide.md#10b041ef-c177-498a-93ed-44b3441ab152 "Microsoft Azure Storage") of the [DBMS Guide](virtual-machines-linux-sap-dbms-guide.md "SAP NetWeaver on Linux virtual machines \(VMs\) – DBMS Deployment Guide")
...
Depending on your on-premises network configuration, it might be required to configure the proxy on your virtual machine if it is connected to your on-premises network via VPN or Express Route. Otherwise the virtual machine might not be able to access the internet and therefore cannot download the required extensions or collect monitoring data. Please see chapter [Configure Proxy](virtual-machines-linux-sap-deployment-guide.md#baccae00-6f79-4307-ade4-40292ce4e02d "Configure Proxy") of this document.

#### Join Domain (Windows only)
In the case that the deployment in Azure is connected to the on-premises AD/DNS via Azure Site-to-Site or Express Route (also referenced as Cross-Premises in the [Planning and Implementation Guide](virtual-machines-linux-sap-planning-guide.md "SAP NetWeaver on Linux virtual machines \(VMs\) – Planning and Implementation Guide")), it is expected that the VM is joining an on-premises domain. Considerations of this step are described in chapter [Join VM into on-premises Domain (Windows only)](virtual-machines-linux-sap-deployment-guide.md#31d9ecd6-b136-4c73-b61e-da4a29bbc9cc "Join VM into on-premises Domain - Windows only") of this document.

#### <a name="ec323ac3-1de9-4c3a-b770-4ff701def65b"></a>Configure Monitoring
Configure the Azure Enhanced Monitoring Extension for SAP as described in chapter [Configure Azure Enhanced Monitoring Extension for SAP](virtual-machines-linux-sap-deployment-guide.md#d98edcd3-f2a1-49f7-b26a-07448ceb60ca "Configure Azure Enhanced Monitoring Extension for SAP") of this document.

Check the prerequisites for SAP Monitoring for required minimum versions of SAP Kernel and SAP Host Agent in the resources listed in chapter [SAP Resources](virtual-machines-linux-sap-deployment-guide.md#42ee2bdb-1efc-4ec7-ab31-fe4c22769b94 "SAP Resources") of this document.

#### Monitoring Check
Check if the monitoring is working as described in chapter [Checks and Troubleshooting for End-to-End Monitoring Setup for SAP on Azure](virtual-machines-linux-sap-deployment-guide.md#564adb4f-5c95-4041-9616-6635e83a810b "Checks and Troubleshooting for End-to-End Monitoring Setup for SAP on Azure").
...
As described in the [Planning and Implementation Guide](virtual-machines-linux-sap-planning-guide.md "SAP NetWeaver on Linux virtual machines \(VMs\) – Planning and Implementation Guide") already in detailed steps there is a way to prepare and create a custom image and use it to create multiple new VMs. 
The sequence of steps in the flow chart would look like:

![Flowchart of VM deployment for SAP systems using a VM image in Private Marketplace](./media/virtual-machines-linux-sap-deployment-guide/300-deploy-private-image.png)

Following the flowchart the following steps need to be executed:

#### Create virtual machine
To create a deployment using a private OS image through the Azure Portal, use one of the SAP templates published on the [azure-quickstart-templates github repository](https://github.com/Azure/azure-quickstart-templates).
You can also create a virtual machine using the [PowerShell](virtual-machines-windows-upload-image.md) manually. 

* [2-tier configuration (only one virtual machine) template](https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2FAzure%2Fazure-quickstart-templates%2Fmaster%2Fsap-2-tier-user-image%2Fazuredeploy.json)
  Use this template if you want to create a 2-tier system using only one virtual machine and your own OS image.
* [3-tier configuration (multiple virtual machines) template](https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2FAzure%2Fazure-quickstart-templates%2Fmaster%2Fsap-3-tier-user-image%2Fazuredeploy.json)
...
Configure Azure Monitoring Extension for SAP as described in chapter [Configure Azure Enhanced Monitoring Extension for SAP](virtual-machines-linux-sap-deployment-guide.md#d98edcd3-f2a1-49f7-b26a-07448ceb60ca "Configure Azure Enhanced Monitoring Extension for SAP") of this document.
Check the prerequisites for SAP Monitoring for required minimum versions of SAP Kernel and SAP Host Agent in the resources listed in chapter [SAP Resources](virtual-machines-linux-sap-deployment-guide.md#42ee2bdb-1efc-4ec7-ab31-fe4c22769b94 "SAP Resources") of this document.
...
> ![Windows](./media/virtual-machines-linux-sap-shared/Windows.png) Windows
> 
> [http://blogs.msdn.com/b/wats/archive/2014/02/17/bginfo-guest-agent-extension-for-azure-vms.aspx](http://blogs.msdn.com/b/wats/archive/2014/02/17/bginfo-guest-agent-extension-for-azure-vms.aspx)
> 
> ![Linux](./media/virtual-machines-linux-sap-shared/Linux.png) Linux
> 
...
![Flowchart of VM deployment for SAP systems using a VM Disk](./media/virtual-machines-linux-sap-deployment-guide/400-deploy-using-disk.png)

Assuming that the Disk is already uploaded and defined in Azure (see [Planning and Implementation Guide](virtual-machines-linux-sap-planning-guide.md "SAP NetWeaver on Linux virtual machines \(VMs\) – Planning and Implementation Guide")), follow these steps

#### Create virtual machine
To create a deployment using a private OS disk through the Azure Portal, use the SAP template published on the [azure-quickstart-templates github repository](https://github.com/Azure/azure-quickstart-templates). You can also create a virtual machine using the [PowerShell or Azure CLI manually.

* [2-tier configuration (only one virtual machine) template](https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2FAzure%2Fazure-quickstart-templates%2Fmaster%2Fsap-2-tier-user-disk%2Fazuredeploy.json)
  * Use this template if you want to create a 2-tier system using only one virtual machine.
...
#### Install VM Agent
The Linux Agent must already be installed in the OS disk if you want to use the templates above. Otherwise the deployment will fail. Download and Install the VM Agent in the VM as described in chapter [Download, Install and enable Azure VM Agent](virtual-machines-linux-sap-deployment-guide.md#c7cbb0dc-52a4-49db-8e03-83e7edc2927d "Download, Install and enable Azure VM Agent") of this document.
...
#### Configure Monitoring
Configure Azure Enhanced Monitoring Extension for SAP as described in chapter [Configure Azure Enhanced Monitoring Extension for SAP](virtual-machines-linux-sap-deployment-guide.md#d98edcd3-f2a1-49f7-b26a-07448ceb60ca "Configure Azure Enhanced Monitoring Extension for SAP") of this document.
...
* Update the monitoring infrastructure by following the steps explained in chapter [Configure Azure Enhanced Monitoring Extension for SAP](virtual-machines-linux-sap-deployment-guide.md#d98edcd3-f2a1-49f7-b26a-07448ceb60ca "Configure Azure Enhanced Monitoring Extension for SAP") of this document. A re-run of the script described in this chapter will detect that a monitoring configuration is deployed and will perform the necessary changes to the monitoring configuration. 
...
> ![Linux](./media/virtual-machines-linux-sap-shared/Linux.png) Linux
> 
> Please follow the steps in [this article](virtual-machines-linux-update-agent.md) to update the Azure Linux Agent. 
> 
> 
...
* Go to [https://azure.microsoft.com/downloads/](https://azure.microsoft.com/downloads/)
...
![Installation screen for Azure PowerShell cmdlets](./media/virtual-machines-linux-sap-deployment-guide/500-install-powershell.png)
<a name="figure-5"></a>

* Press Install and accept the EULA.

Check frequently whether the PowerShell cmdlets have been updated. Usually there are updates on a monthly period. The easiest way to do this is to follow the installation steps as described above up to the installation screen shown in [this](virtual-machines-linux-sap-deployment-guide.md#figure-5) figure. In this screen, the release date of the cmdlets is shown as well as the actual release number. Unless stated differently in SAP Notes [1928533](https://service.sap.com/sap/support/notes/1928533) or [2015553](https://service.sap.com/sap/support/notes/2015553), it is recommended to work with the latest version of Azure PowerShell cmdlets.
...
The result should be presented as shown below in [this](virtual-machines-linux-sap-deployment-guide.md#figure-6) figure.

![Result of Azure PS cmdlet version check](./media/virtual-machines-linux-sap-deployment-guide/600-powershell-version.png)
<a name="figure-6"></a>

If the Azure cmdlet version installed on the desktop/laptop is the current one, the first screen after starting the Microsoft Web Platform Installer will look slightly different compared to the one shown in [this](virtual-machines-linux-sap-deployment-guide.md#figure-5) figure.

Please notice the red circle below in the [figure](virtual-machines-linux-sap-deployment-guide.md#figure-7) below.

![Installation screen for Azure PowerShell cmdlets indicating that most recent release of Azure PS cmdlets are installed](./media/virtual-machines-linux-sap-deployment-guide/700-install-powershell-installed.png)
<a name="figure-7"></a>

If the screen looks as [above](virtual-machines-linux-sap-deployment-guide.md#figure-7), indicating that the most recent Azure cmdlet version is already installed, there is no need to continue with the installation. In this case you can ‘Exit’ the installation at this stage.
...
The result should be presented as shown below in [this](virtual-machines-linux-sap-deployment-guide.md#0ad010e6-f9b5-4c21-9c09-bb2e5efb3fda) figure.

![Result of Azure CLI version check](./media/virtual-machines-linux-sap-deployment-guide/760-azure-cli-version.png)
...
* Download the Azure VM Agent:
  * Download the Azure VM Agent installer package from: [https://go.microsoft.com/fwlink/?LinkId=394789](https://go.microsoft.com/fwlink/?LinkId=394789)
...
1. Open gpedit.msc
2. Navigate to Computer Configuration –> Administrative Templates -> Windows Components -> Internet Explorer and enable “Make proxy settings per-machine (rather than per-user)
...
6. Enable "Use a proxy server for your LAN" and enter the proxy host and port
...
For more details about User Defined Routes, see [this article](../virtual-network/virtual-networks-udr-overview.md).

![Forced Tunneling](./media/virtual-machines-linux-sap-deployment-guide/50-forced-tunneling-suse.png)

### <a name="d98edcd3-f2a1-49f7-b26a-07448ceb60ca"></a>Configure Azure Enhanced Monitoring Extension for SAP
Once the VM is prepared as described in chapter [Deployment Scenarios of VMs for SAP on Microsoft Azure](virtual-machines-linux-sap-deployment-guide.md#b3253ee3-d63b-4d74-a49b-185e76c4088e "Deployment Scenarios of VMs for SAP on Microsoft Azure"), the Azure VM Agent is installed in the machine. The next important step is to deploy the Azure Enhanced Monitoring Extension for SAP, which is available in the Azure Extension Repository in the global datacenters of Microsoft Azure. For more details, please check the [Planning and Implementation Guide](virtual-machines-linux-sap-planning-guide.md#6f0a47f3-a289-4090-a053-2521618a28c3 "Azure Monitoring Solution for SAP"). 

You can use Azure PowerShell or Azure CLI to install and configure the Azure Enhanced Monitoring Extension for SAP. Please read chapter [Azure PowerShell](virtual-machines-linux-sap-deployment-guide.md#987cf279-d713-4b4c-8143-6b11589bb9d4 "Azure PowerShell") if you want to install the extension on a Windows or Linux VM using a Windows machine. For installing the extension on a Linux VM using a Linux desktop read chapter [Azure CLI](virtual-machines-linux-sap-deployment-guide.md#408f3779-f422-4413-82f8-c57a23b4fc2f "Azure CLI").
...
* Make sure that you have installed the latest version of the Microsoft Azure PowerShell cmdlet. See chapter [Deploying Azure PowerShell cmdlets](virtual-machines-linux-sap-deployment-guide.md#604bcec2-8b6e-48d2-a944-61b0f5dee2f7 "Deploying Azure PowerShell cmdlets") of this document.  
...
Please read [this MSDN article](https://msdn.microsoft.com/library/azure/mt670598.aspx) for more information about the Set-AzureRmVMAEMExtension.

![Result screen of successful execution of SAP specific Azure cmdlet Set-AzureRmVMAEMExtension](./media/virtual-machines-linux-sap-deployment-guide/900-cmd-update-executed.png)
...
1. Install Azure CLI as described in [this](../xplat-cli-install.md) article
...
Therefore, execute the Readiness check for the Azure Enhanced Monitoring Extension for SAP as described in chapter [Readiness Check for Azure Enhanced Monitoring for SAP](virtual-machines-linux-sap-deployment-guide.md#bb61ce92-8c5c-461f-8c53-39f5e5ed91f2 "Readiness Check for Azure Enhanced Monitoring for SAP"). If the result of this check is positive and you get all relevant performance counters, the Azure monitoring has been setup successfully. In this case, proceed with the installation of the SAP Host Agent as described in the SAP Notes listed in chapter [SAP Resources](virtual-machines-linux-sap-deployment-guide.md#42ee2bdb-1efc-4ec7-ab31-fe4c22769b94 "SAP Resources") of this document. If the result of the Readiness check indicates missing counters, proceed executing the Health check for the Azure Monitoring Infrastructure as described in chapter [Health check for Azure Monitoring Infrastructure Configuration](virtual-machines-linux-sap-deployment-guide.md#e2d592ff-b4ea-4a53-a91a-e5521edb6cd1 "Health check for Azure Monitoring Infrastructure Configuration"). In case of any problem with the Azure Monitoring Configuration, check chapter [Further troubleshooting of Azure Monitoring infrastructure for SAP](virtual-machines-linux-sap-deployment-guide.md#fe25a7da-4e4e-4388-8907-8abc2d33cfd8 "Further troubleshooting of Azure Monitoring infrastructure for SAP") for further help on troubleshooting.
...
![Properties of Service running the Azure Enhanced Monitoring Extension for SAP](./media/virtual-machines-linux-sap-deployment-guide/1000-service-properties.png)
...
If the Azure Enhanced Monitoring Extension is not installed or the service ‘AzureEnhancedMonitoring’ is not running, the extension has not been configured correctly. In this case, follow chapter [Further troubleshooting of Azure Monitoring infrastructure for SAP](virtual-machines-linux-sap-deployment-guide.md#fe25a7da-4e4e-4388-8907-8abc2d33cfd8 "Further troubleshooting of Azure Monitoring infrastructure for SAP") for detailed instructions how to redeploy the extension.
...
![Output of health check by executing azperflib.exe indicating that no problems exist](./media/virtual-machines-linux-sap-deployment-guide/1100-azperflib.png)
<a name="figure-11"></a>

Check the result returned for the output of the amount of ‘Counters total’, which are reported as empty and for the ‘Health check’ as shown above in the figure [above](virtual-machines-linux-sap-deployment-guide.md#figure-11).
...
* Install the SAP Host Agent as described in SAP Note [1031096](https://service.sap.com/sap/support/notes/1031096) and check the output of saposcol
...
If any of the checks above fail, follow chapter [Further troubleshooting of Azure Monitoring infrastructure for SAP](virtual-machines-linux-sap-deployment-guide.md#fe25a7da-4e4e-4388-8907-8abc2d33cfd8 "Further troubleshooting of Azure Monitoring infrastructure for SAP") for detailed instructions how to redeploy the extension.

### <a name="e2d592ff-b4ea-4a53-a91a-e5521edb6cd1"></a>Health check for Azure Monitoring Infrastructure Configuration
If some of the monitoring data is not delivered correctly as indicated by the test described in chapter [Readiness Check for Azure Enhanced Monitoring for SAP](virtual-machines-linux-sap-deployment-guide.md#bb61ce92-8c5c-461f-8c53-39f5e5ed91f2 "Readiness Check for Azure Enhanced Monitoring for SAP") above, execute the Test-AzureRmVMAEMExtension cmdlet to test if the current configuration of the Azure Monitoring infrastructure and the Monitoring extension for SAP is correct.

In order to test the monitoring configuration, please execute the following sequence:

* Make sure that you have installed the latest version of the Microsoft Azure PowerShell cmdlet as described in chapter [Deploying Azure PowerShell cmdlets](virtual-machines-linux-sap-deployment-guide.md#604bcec2-8b6e-48d2-a944-61b0f5dee2f7 "Deploying Azure PowerShell cmdlets") of this document.
* Run the following PowerShell cmdlet. For a list of available environments, run commandlet Get-AzureRmEnvironment. If you want to use public Azure, your environment is AzureCloud. For Azure in China, select AzureChinaCloud.
...
![Input screen of SAP specific Azure cmdlet Test-VMConfigForSAP_GUI](./media/virtual-machines-linux-sap-deployment-guide/1200-cmd-test-login.png)
...
![Output of successful test of Azure Monitoring Infrastructure for SAP](./media/virtual-machines-linux-sap-deployment-guide/1300-cmd-test-executed.png)

Make sure that every check is marked with OK. If some of the checks are not ok, please execute the update cmdlet as described in chapter [Configure Azure Enhanced Monitoring Extension for SAP](virtual-machines-linux-sap-deployment-guide.md#d98edcd3-f2a1-49f7-b26a-07448ceb60ca "Configure Azure Enhanced Monitoring Extension for SAP") of this document. Wait another 15 minutes and perform the checks described in chapter [Readiness Check for Azure Enhanced Monitoring for SAP](virtual-machines-linux-sap-deployment-guide.md#bb61ce92-8c5c-461f-8c53-39f5e5ed91f2 "Readiness Check for Azure Enhanced Monitoring for SAP") and [Health check for Azure Monitoring Infrastructure Configuration](virtual-machines-linux-sap-deployment-guide.md#e2d592ff-b4ea-4a53-a91a-e5521edb6cd1 "Health check for Azure Monitoring Infrastructure Configuration") again. If the checks still indicate a problem with some or all counters, please proceed to chapter [Further troubleshooting of Azure Monitoring infrastructure for SAP](virtual-machines-linux-sap-deployment-guide.md#fe25a7da-4e4e-4388-8907-8abc2d33cfd8 "Further troubleshooting of Azure Monitoring infrastructure for SAP").

### <a name="fe25a7da-4e4e-4388-8907-8abc2d33cfd8"></a>Further troubleshooting of Azure Monitoring infrastructure for SAP
#### ![Windows](./media/virtual-machines-linux-sap-shared/Windows.png) Azure performance counters do not show up at all
...
Azperflib.exe:    The azperlib.exe output throws an error as shown in the [figure below](virtual-machines-linux-sap-deployment-guide.md#figure-14).

![Execution of azperflib.exe indicates that the service of the Azure Enhanced Monitoring Extension for SAP is not running](./media/virtual-machines-linux-sap-deployment-guide/1400-azperflib-error-servicenotstarted.png)
<a name="figure-14"></a>

###### Solution
If the service does not exist as shown in the [figure above](virtual-machines-linux-sap-deployment-guide.md#figure-14), the Azure Monitoring Extension for SAP has not been installed correctly. Redeploy the extension according to the steps described for your deployment scenario in chapter [Deployment Scenarios of VMs for SAP on Microsoft Azure](virtual-machines-linux-sap-deployment-guide.md#b3253ee3-d63b-4d74-a49b-185e76c4088e "Deployment Scenarios of VMs for SAP on Microsoft Azure"). 
...
###### Solution
Bad configuration. Re-enable the monitoring extension for the VM as described in chapter [Configure Azure Enhanced Monitoring Extension for SAP](virtual-machines-linux-sap-deployment-guide.md#d98edcd3-f2a1-49f7-b26a-07448ceb60ca "Configure Azure Enhanced Monitoring Extension for SAP").

#### ![Windows](./media/virtual-machines-linux-sap-shared/Windows.png) Some Azure performance counters are missing
...
If troubleshooting using SAP Note [1999351](https://service.sap.com/sap/support/notes/1999351) did not help, please re-run the configuration script Set-AzureRmVMAEMExtension. You may have to wait for an hour because storage analytics or diagnostics counters may not be created immediately after they are enabled. If the problem still exists, open an SAP customer support message on the component BC-OP-NT-AZR.

#### ![Linux](./media/virtual-machines-linux-sap-shared/Linux.png) Azure performance counters do not show up at all
...
#### ![Linux](./media/virtual-machines-linux-sap-shared/Linux.png) Some Azure performance counters are missing
...
For a complete and up to date list of known issues please see SAP Note [1999351](https://service.sap.com/sap/support/notes/1999351) containing additional troubleshooting information for the Enhanced Azure Monitoring for SAP.

If troubleshooting using SAP Note [1999351](https://service.sap.com/sap/support/notes/1999351) did not help, please re-run the configuration script Set-AzureRmVMAEMExtension as described in chapter [Configure Azure Enhanced Monitoring Extension for SAP](virtual-machines-linux-sap-deployment-guide.md#d98edcd3-f2a1-49f7-b26a-07448ceb60ca "Configure Azure Enhanced Monitoring Extension for SAP"). You may have to wait for an hour because storage analytics or diagnostics counters may not be created immediately after they are enabled. If the problem still exists, open an SAP customer support message on the component BC-OP-NT-AZR for Windows or BC-OP-LNX-AZR for a Linux virtual machine.
