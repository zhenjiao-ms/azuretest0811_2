---
title: Always Encrypted - Protect sensitive data in Azure SQL Database with database encryption
...
10. Leave **SHOW** set to **Microsoft Apps** and then locate and select **Windows Azure Service Management** and click the check-mark to continue.
11. On the **Windows Azure Service Mana...** line click the **Delegated Permissions** dropdown and select **Access Azure Service Management**.
12. Click **SAVE** (at the bottom).
13. After the save completes, locate and copy the key value in the **keys** section (you will need this value in your code later). 
...
4. **Master Key Configuration** (CMK)
   ...
   2. Select the desired key vault from the dropdown.
   3. Click **Next**.
...
1. Install the following NuGet packages by clicking **Tools** > **NuGet Package Manager** > **Package Manager Console**.
