---
title: Deploy a split-merge service | Microsoft Azure
...
2. Ensure that your Azure SQL DB server allows Azure Services to connect to it. In the portal, in the **Firewall Settings**, ensure that the **Allow access to Azure Services** setting is set to **On**. Click the "save" icon.
    ...
    ![Allowed services](./media/sql-database-elastic-scale-configure-deploy-split-and-merge/allowed-services.png)
...
4. Choose the staging environment, then click **Upload a new staging deployment**.
    ...
    ![Staging](./media/sql-database-elastic-scale-configure-deploy-split-and-merge/staging.png)
...
![Upload](./media/sql-database-elastic-scale-configure-deploy-split-and-merge/upload.png)
