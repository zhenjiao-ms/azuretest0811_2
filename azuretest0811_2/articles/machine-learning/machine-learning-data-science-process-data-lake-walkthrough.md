---
title: 'Scalable Data Science in Azure Data Lake: An end-to-end Walkthrough | Microsoft Azure'
...
The location of the data in the Azure blob is referenced as **wasb://container_name@blob_storage_account_name.blob.core.windows.net/blob_name** and can be extracted using **Extractors.Csv()**. Substitute your own container name and storage account name in following scripts for container*name@blob_storage_account_name in the wasb address. Since the file names are in same format, we can use **trip\_data*{\*\}.csv** to read in all 12 trip files. 
