---
title: Parallel Bulk Data Import Using SQL Partition Tables | Microsoft Azure
...
This document describes how to build partitioned table(s) for fast parallel bulk importing of data to a SQL Server database. For big data loading/transfer to an SQL database, importing data to the SQL DB and subsequent queries can be improved by using *Partitioned Tables and Views*. 
...
* Add one or more files (as needed) to each database filegroup
  ...
  > [!NOTE]
  > Specify the target filegroup which hold data for this partition and the physical database file name(s) where the filegroup data will be stored.
  > 
  > 
