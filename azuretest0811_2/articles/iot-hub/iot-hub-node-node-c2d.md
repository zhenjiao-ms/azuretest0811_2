...
## Introduction
Azure IoT Hub is a fully managed service that helps enable reliable and secure bi-directional communications between millions of IoT devices and an application back end. The [Get started with IoT Hub](iot-hub-node-node-getstarted.md) tutorial shows how to create an IoT hub, provision a device identity in it, and code a simulated device that sends device-to-cloud messages.

This tutorial builds on [Get started with IoT Hub](iot-hub-node-node-getstarted.md). It shows you how to:
...
* **SimulatedDevice**, a modified version of the app created in [Get started with IoT Hub](iot-hub-node-node-getstarted.md), which connects to your IoT hub and receives cloud-to-device messages.
...
In this section, you'll modify the simulated device application you created in [Get started with IoT Hub](iot-hub-node-node-getstarted.md) to receive cloud-to-device messages from the IoT hub.
...
4. Add the following `require` statements at the start of the **SendCloudToDeviceMessage.js** file:
    ...
    ```
5. Add the following code to **SendCloudToDeviceMessage.js** file and replace the connection string placeholder value with the connection string for the IoT hub you created in the [Get started with IoT Hub](iot-hub-node-node-getstarted.md) tutorial and the target device placeholder with the device id of the device you added in the [Get started with IoT Hub](iot-hub-node-node-getstarted.md) tutorial:
...
2. At a command-prompt in the **sendcloudtodevicemessage** folder, run the following command to send a cloud-to-device message and wait for the acknowledgment feedback:
    ...
    ![Run the app to send the c2d command](media/iot-hub-node-node-c2d/sendc2d.png)
