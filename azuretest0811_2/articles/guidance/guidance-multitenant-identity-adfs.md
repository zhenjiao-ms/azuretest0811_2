---
title: Federating with a customer's AD FS | Microsoft Azure
...
One option is for these customers to sync their on-premise AD with Azure AD, using [Azure AD Connect](../active-directory/active-directory-aadconnect.md). However, some customers may be unable to use this approach, due to corporate IT policy or other reasons. In that case, another option is to federate through Active Directory Federation Services (AD FS).
...
* The customer and the SaaS provider must set up [federation trust](https://technet.microsoft.com/library/cc770993\(v=ws.11\).aspx). This is a manual process.

There are three main roles in the trust relation:

* The customer's AD FS is the [account partner](https://technet.microsoft.com/library/cc731141\(v=ws.11\).aspx), responsible for authenticating users from the customer's AD, and creating security tokens with user claims.
* The SaaS provider's AD FS is the [resource partner](https://technet.microsoft.com/library/cc731141\(v=ws.11\).aspx), which trusts the account partner and receives the user claims.
...
For an example of using WS-Federation with ASP.NET 4, see the [active-directory-dotnet-webapp-wsfederation sample](https://github.com/Azure-Samples/active-directory-dotnet-webapp-wsfederation).
...
| authenticationinstant |[Authentication instant](https://msdn.microsoft.com/library/system.security.claims.claimtypes.authenticationinstant%28v=vs.110%29.aspx). The time at which authentication occurred. |
| c_hash |Code hash value. This is a hash of the token contents. |
| exp |[Expiration time](http://tools.ietf.org/html/draft-ietf-oauth-json-web-token-25#section-4.1.4). The time after which the token will no longer be accepted. |
| iat |[Issued at](http://tools.ietf.org/html/draft-ietf-oauth-json-web-token-25#section-4.1.6). The time when the token was issued. |
| iss |Issuer. The value of this claim is always the resource partner's AD FS. |
| name |User name. Example: `john@corp.fabrikam.com`. |
| nameidentifier |[Name identifier](https://msdn.microsoft.com/library/system.security.claims.claimtypes.nameidentifier\(v=vs.110\).aspx). The identifier for the name of the entity for which the token was issued. |
...
To set up a similar topology in Azure requires the use of Virtual networks, NSG’s, azure VM’s and availability sets. For more details, see [Guidelines for Deploying Windows Server Active Directory on Azure Virtual Machines](MSDN.md).
...
With this configuration, end users can type in their organizational account, and AD FS automatically selects the corresponding claims provider. See [Customizing the AD FS Sign-in Pages](https://technet.microsoft.com/library/dn280950.aspx), under the section "Configure Identity Provider to use certain email suffixes".
...
## Next steps
* Read the next article in this series: [Using client assertion to get access tokens from Azure AD](guidance-multitenant-identity-client-assertion.md)
