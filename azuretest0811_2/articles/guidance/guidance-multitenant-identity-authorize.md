---
title: Authorization in multitenant applications | Microsoft Azure
...
Our  [reference implementation](guidance-multitenant-identity-tailspin.md) is an ASP.NET Core 1.0 application. In this article we'll look at two general approaches to authorization, using the authorization APIs provided in ASP.NET Core 1.0.
...
The [Tailspin Surveys](guidance-multitenant-identity-tailspin.md) application defines the following roles:
...
For a discussion of how to define and manage roles, see [Application roles](guidance-multitenant-identity-app-roles.md).

Regardless of how you manage the roles, your authorization code will look similar. ASP.NET Core 1.0 introduces an abstraction called [authorization policies](https://docs.asp.net/en/latest/security/authorization/policies.html). With this feature, you define authorization policies in code, and then apply those policies to controller actions. The policy is decoupled from the controller.
...
## Next steps
* Read the next article in this series: [Securing a backend web API in a multitenant application](guidance-multitenant-identity-web-api.md)
* To learn more about resource based authorization in ASP.NET 1.0 Core, see [Resource Based Authorization](https://docs.asp.net/en/latest/security/authorization/resourcebased.html).
