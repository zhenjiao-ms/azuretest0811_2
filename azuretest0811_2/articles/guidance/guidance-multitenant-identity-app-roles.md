...
Application roles are used to assign permissions to users. For example, the [Tailspin Surveys](guidance-multitenant-identity-tailspin.md) application defines the following roles:
...
You can see that roles ultimately get translated into permissions, during [authorization](guidance-multitenant-identity-authorize.md). But the first question is how to assign and manage roles. We identified three main options:
...
* If you have a backend web API, which is separate from the web app, then role assignments for the web app don't apply to the web API. For more discussion of this point, see [Securing a backend web API](guidance-multitenant-identity-web-api.md).

### Implementation
**Define the roles.** The SaaS provider declares the app roles in the [application manifest](../active-directory/active-directory-application-manifest.md). For example, here is the manifest entry for the Surveys app:
...
There are many existing examples for this approach. For example, see [Create an ASP.NET MVC app with auth and SQL DB and deploy to Azure App Service](../app-service-web/web-sites-dotnet-deploy-aspnet-mvc-app-membership-oauth-sql-database.md).

## Next steps
* Read the next article in this series: [Role-based and resource-based authorization in multitenant applications](guidance-multitenant-identity-authorize.md)
