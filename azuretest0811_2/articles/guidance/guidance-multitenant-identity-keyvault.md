---
title: Using Key Vault to protect application secrets | Microsoft Azure
...
A more secure option is to store these secrets in [Azure Key Vault](https://azure.microsoft.com/services/key-vault/). Key Vault is a cloud-hosted service for managing cryptographic keys and other secrets. This article shows how to use Key Vault to store configuration settings for you app.

In the [Tailspin Surveys](guidance-multitenant-identity-tailspin.md) application, the following settings are secret:

* The database connection string.
* The Redis connection string.
* The client secret for the web application.

To store configuration secrets in Key Vault, the Surveys application implements a custom configuration provider, which hooks into the ASP.NET Core 1.0 [configuration system](https://docs.asp.net/en/latest/fundamentals/configuration.html). The custom provider reads configuration settings from Key Vault on startup.

The Surveys application loads configuration settings from the following places:

* The appsettings.json file
* The [user secrets store](http://go.microsoft.com/fwlink/?LinkID=532709) (development environment only; for testing)
...
> 
> The Key Vault provider is currently not supported for .NET Core, because it requires the [Microsoft.Azure.KeyVault](https://www.nuget.org/packages/Microsoft.Azure.KeyVault/) package.
> 
> 

At startup, the application reads settings from every registered configuration provider, and uses them to populate a strongly typed options object. (For more information, see [Using Options and configuration objects](https://docs.asp.net/en/latest/fundamentals/configuration.html#using-options-and-configuration-objects).)

## Implementation
The [KeyVaultConfigurationProvider](https://github.com/Azure-Samples/guidance-identity-management-for-multitenant-apps/blob/master/src/Tailspin.Surveys.Configuration.KeyVault/KeyVaultConfigurationProvider.cs) class is a configuration provider that plugs into the ASP.NET Core 1.0 [configuration system](https://docs.asp.net/en/latest/fundamentals/configuration.html).
...
> [!NOTE]
> [Tags](https://msdn.microsoft.com/library/azure/dn903623.aspx#BKMK_Keytags) are optional metadata stored with a key. Tags are used here because key names cannot contain colon (:) characters.
> 
> 
...
* Install the [Azure Resource Manager Cmdlets](https://msdn.microsoft.com/library/mt125356.aspx).
* Configure the Surveys application as described in [Running the Surveys application](https://github.com/Azure-Samples/guidance-identity-management-for-multitenant-apps/blob/master/docs/running-the-app.md).
...
2. Run SetupKeyVault.ps again, with the following parameters:
   ...
   * web api client ID = The client ID for the Surveys.WebAPI application.
     ...
     This script authorizes the web app and web API to retrieve secrets from your key vault. See [Get started with Azure Key Vault](../key-vault/key-vault-get-started.md#authorize) for more information.
...
2. In [Tailspin.Surveys.Web/Startup.cs](https://github.com/Azure-Samples/guidance-identity-management-for-multitenant-apps/blob/master/src/Tailspin.Surveys.Web/Startup.cs), locate the following code block and uncomment it.
    ...
    //#endif
    ```
3. In [Tailspin.Surveys.WebAPI/Startup.cs](https://github.com/Azure-Samples/guidance-identity-management-for-multitenant-apps/blob/master/src/Tailspin.Surveys.WebAPI/Startup.cs), locate the following code block and uncomment it.
    ...
    //#endif
    ```
4. In [Tailspin.Surveys.Web/Startup.cs](https://github.com/Azure-Samples/guidance-identity-management-for-multitenant-apps/blob/master/src/Tailspin.Surveys.Web/Startup.cs), locate the code that registers the `ICredentialService`. Uncomment the line that uses `CertificateCredentialService`, and comment out the line that uses `ClientCredentialService`:
